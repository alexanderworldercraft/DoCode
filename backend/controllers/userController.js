...
export const userController = {
  async createAdmin(request, reply) {
    try {
      ...
      if (!requester || requester.GradeID !== 1) {
        return reply.status(403).send({ error: 'Seul un super administrateur peut créer un administrateur.' });
      }
      ...
      for await (const part of parts) {
        if (part.file) {
          ...
          const uploadDir = path.join(__dirname, '../uploads/pp');
  ...
  async login(request, reply) {
    try {
      ...
      // Récupérer l'utilisateur par surnom ou email.
      const user = await userRepository.getUserBySurnomOrEmailIdentifier(surnom);
      if (!user) {
      ...
      if (user.GradeID !== 1 && user.GradeID !== 2) {
